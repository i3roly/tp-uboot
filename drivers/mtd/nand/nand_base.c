...
ident_done:
	...
	/* Check if it is MXIC MX30LF1GE8AB ECC-Free Nand */
        if((id_data[0] == NAND_MFR_MXIC) && (id_data[1] == 0xF1)) {
			if (id_data[4] & 0x80) { /* Internal ECC enabled */
				chip->options |= NAND_MXIC_ECC_FREE;
			chip->ecc.mode = NAND_ECC_BENAND;
		} else 
			mtd->oobsize = 32 * mtd->writesize >> 9; /* !ECC-Free Nand */
	}
