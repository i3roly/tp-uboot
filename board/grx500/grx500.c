...
#define GPIO_TPLED_POWER 21
#define GPIO_TPLED_WLAN2G 1
#define GPIO_TPLED_WLAN5G 4
#define GPIO_TPLED_OWAN 6
#define GPIO_TPLED_BWAN 7
#define GPIO_TPLED_LAN 9
#define GPIO_TPLED_USB3 11

extern u32 get_cpu_id(void);
int tp_led_all_light();
int board_tp_led_gpio_init(void);
int tp_led_all_close_butpower();
int tp_led_all_light_owan();
int tp_led_close_bwan();
...
//Set Orange Color R:G:B = half bright of 255:165:0
	...
	/*Writen by zhuyu@18.03.23*/
	board_tp_led_gpio_init();
	tp_led_all_light();
	udelay (800000);
	tp_led_close_bwan();
	tp_led_all_light_owan();
	udelay (800000);
	tp_led_all_close_butpower();
	/*end*/
}
...
int board_tp_led_gpio_init(void)
{
	int gpio_map = (1 << GPIO_TPLED_POWER) |
					(1 << GPIO_TPLED_WLAN2G) |
					(1 << GPIO_TPLED_WLAN5G) |
					(1 << GPIO_TPLED_OWAN) |
					(1 << GPIO_TPLED_BWAN) |
	...
	config_gpio(GPIO_TPLED_POWER, GPIO_OUTPUT, 0);
	config_gpio(GPIO_TPLED_WLAN2G, GPIO_OUTPUT, 0);
	config_gpio(GPIO_TPLED_WLAN5G, GPIO_OUTPUT, 0);
	config_gpio(GPIO_TPLED_OWAN, GPIO_OUTPUT, 0);
	config_gpio(GPIO_TPLED_BWAN, GPIO_OUTPUT, 0);
	config_gpio(GPIO_TPLED_LAN, GPIO_OUTPUT, 0);
	config_gpio(GPIO_TPLED_USB3, GPIO_OUTPUT, 0);

	REG32(GPIO_FUNCTION_OUTCLR_0) = gpio_map;
	REG32(GPIO_FUNCTION_DIRSET_0) = gpio_map;
}
...
int tp_led_all_light()
{
	...
	REG32(GPIO_FUNCTION_OUTSET_0) = gpio_map;
}
...
int tp_led_close_bwan()
{
	int gpio_map = 	(1 << GPIO_TPLED_BWAN);

	REG32(GPIO_FUNCTION_DIRCLR_0) = gpio_map;
}

int tp_led_all_close_butpower()
{
	int gpio_map = 	(1 << GPIO_TPLED_WLAN2G) |
					(1 << GPIO_TPLED_WLAN5G) |
					(1 << GPIO_TPLED_OWAN) |
					(1 << GPIO_TPLED_BWAN) |
					(1 << GPIO_TPLED_LAN) |
